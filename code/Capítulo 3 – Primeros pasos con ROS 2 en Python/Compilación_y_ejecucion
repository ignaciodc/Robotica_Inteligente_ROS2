...
### Explicación de la secuencia de instrucciones
* Inicialmente, se debe acceder al directorio donde se encuentra el paquete, y se compila el paquete con el comando ```colcon build```. 
Este comando compila todo el workspace, al añadir ```--packages-select``` y el nombre del paquete. Se compilará solo ese paquete, con lo que evitará esperas innecesarias si no se quiere compilar todo el workspace cada vez.
